...
* **Coordinate & Scale Fixes**:
  ...
  * Scaled horizontal coordinates of screen-spanning elements (like the biplane flyby in `GJVIS3.TTM` and `GJVIS5.TTM`, the tanker in `GJVIS6.TTM`, and rescue boats in `WOULDBE.TTM`/`THEEND.TTM`) across the full widescreen viewport width when widescreen mode is enabled. Restricted scaling only to true screen-spanning sprite image slots (e.g. the speedboat or tanker itself) while applying the standard island shift offset (`widescreenOffsetX`) to static island-based sprites in the same TTMs (such as the palm tree trunk overlay in `GJVIS3.TTM` or `WOULDBE.TTM`), preventing graphical misalignment. Johnny's swimming/climbing animation (`JOHNWOUL.BMP`) and the rope ladder in `WOULDBE.TTM` are scaled to ensure they remain perfectly aligned with the boat during the boarding sequence, while Johnny's walking/diving (`JOHNWALK.BMP`) and drunk on the island (`DRUNKJON.BMP`) animations remain static/shifted to align correctly with the background and prevent drifting. Also resolved widescreen separation of composite multi-part sprites (like the biplane body and its spinning propeller nose) and line-drawn elements (like the waterskiing rope) by tracking the largest scaled sprite drawn in a tick, calculating a stable widescreen `scaleOffsetX` from its position, and applying it consistently to all drawn sprites, lines, and rectangles on the same thread. This preserves relative offsets and removes back-and-forth jitter. `DRAW_RECT` fills (used for the tanker's hull) now get the same scaling as sprites, computed from each strip's left/right edges rather than scaling `x` and `width` independently - the latter rounds differently strip to strip and was opening a ~1px seam at nearly every hull strip boundary in widescreen mode.
