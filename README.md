...
* **Options Dialog (`/c`)**: Implemented a Raylib-based screensaver configuration window (600x520) for setting options (Sound, Silent Time with custom quiet hours interval, Load Background, Widescreen, Scaling Filter, Password Protection, Software OpenGL, "Start of Day" time setting, and Independent instances). Preferences are saved persistently to `%USERPROFILE%\.johnny_castaway_2026`.
* **Silent Time Schedule**: Added a configurable "Silent Time" mode. Users can set a custom quiet hours interval (e.g. from 22:00 to 07:00). During silent time, the screensaver runs uninterrupted while automatically muting all sound effects. Supports intraday intervals as well as overnight schedules spanning midnight.
...
* **Main Monitor Launch Mode (`/1`, `-1`, `/main`, `-main`, or `-m`)**: Added command-line arguments to restrict the screensaver/application window exclusively to the primary/main monitor instead of spanning across all connected displays or spawning child instances across multiple monitors. This makes video recording, screen sharing, and streaming significantly easier while keeping secondary displays completely free for work or other applications. You can also specify an exact monitor index (e.g. `-m 0` or `-m 1`).
...
* **Graceful Exit & Display/HDR Cleanup**: Replaced direct hard-kills (`os.Exit`) with state-controlled cleanup flags. This ensures proper teardown, cleanly restoring desktop resolution and HDR display states on exit. Added an automatic HDR watchdog (`startHDRWatchdog()`) on Windows that listens for `WM_POWERBROADCAST` monitor resume events (`PBT_APMRESUMEAUTOMATIC` / `PBT_APMRESUMESUSPEND`) to trigger `ChangeDisplaySettingsW(nil, 0)` when displays wake up, forcing the GPU driver to re-initialize HDR tone mapping after monitor sleep.
