...
* **Coordinate & Scale Fixes**:
  ...
  * Scaled horizontal coordinates of screen-spanning elements (like the biplane flyby in `GJVIS3.TTM` and `GJVIS5.TTM`, the tanker in `GJVIS6.TTM`, and rescue boats in `WOULDBE.TTM`/`THEEND.TTM`) across the full widescreen viewport width when widescreen mode is enabled. `DRAW_RECT` fills (used for the tanker's hull) now get the same scaling as sprites, computed from each strip's left/right edges rather than scaling `x` and `width` independently - the latter rounds differently strip to strip and was opening a ~1px seam at nearly every hull strip boundary in widescreen mode.
